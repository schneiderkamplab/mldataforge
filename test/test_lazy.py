...
@pytest.mark.parametrize("fmt,param,compression,trafo", [
    ("jinx", True, None, ["def process(sample): return {'id': sample['id']}"]),
    ("jinx", True, "snappy", ["def process(sample): return {'id': sample['id']}"]),
    ("jinx", False, None, ["def process(sample): return {'id': sample['id']}"]),
    ("jinx", False, "snappy", ["def process(sample): return {'id': sample['id']}"]),
